...
> **Implementation Note:** A DTG credential's `proof` is a `DataIntegrityProof` under either version (see [Base Structure](#base-structure)); a v1.1 credential additionally lists the Data Integrity context `https://w3id.org/security/data-integrity/v2`. Verifiers supporting both v1.1 and v2.0 credentials MUST be able to verify that proof under each version for every cryptosuite they accept. Issuers SHOULD use well-supported cryptosuites and include all necessary contexts.
...
- **Subject–object relationship:** `dtg:witnessed` means that the issuer observed the subject *issue* the credential that `object.digestMultibase` names. Accordingly, `credentialSubject.id` MUST be the DID of that credential's `issuer`, and a verifier holding the referenced credential MUST check that it is. This binds each VWC to one direction of the edge it attests, and it is a check the verifier can run whenever it can run the digest check at all. A statement that a party *presented*, *held*, or *received* a credential — where the observed party is the referenced credential's subject and its issuer may be absent — is a different predicate, with the observed party as its subject, and is defined in the vocabulary rather than expressed by stretching `dtg:witnessed`.
- **Additional members:**
...
> **Editor's note — direction binding.** In WD02 the requirement that the subject be the issuer of the referenced credential was stated only for bidirectional exchanges, and the general schema described the subject as "the observed party". This profile makes it unconditional, because the condition is not in the credential: a verifier holding a VWC and the credential it names cannot tell whether that credential was one half of a reciprocal pair, so a rule that depends on it cannot be checked, and two conformant VWCs naming the same credential could name different parties. Unconditional binding covers the cases raised in review without a second rule: on a membership edge the member is named by the VWC for the member-issued VMC, and if only the grant was witnessed then naming the community is correct, since the member's participation was not observed (compare [Membership Edge Completion](#membership-edge-completion)); a [[ref: VDC]] is likewise a grant and an acceptance, so the delegate is named by the VWC for the acceptance. Witnessing a party *present* or *hold* a credential is a different statement and a different predicate — see the subject–object relationship above. The other normative content of the WD02 VWC section is carried over unchanged: one VWC per direction, `taskContext` REQUIRED, `directed` minimum scope, making the referenced credential available, and the outcome-evidence obligation of [Outcome Interpretability](#outcome-interpretability), which applied to the VWC before and is not narrowed by its becoming a profile.

**Example:**

*Illustrative values: identifiers are truncated, and neither digest is computed from a printed document — [`witness/session/0.1`](https://trusttasks.org/spec/witness/session/0.1) prints no session document to compute `taskDigestMultibase` from. For a statement whose `taskDigestMultibase` reproduces from the documents as printed, see the Vetting Statement in [`vetting/session/0.1`](https://trusttasks.org/spec/vetting/session/0.1).*
