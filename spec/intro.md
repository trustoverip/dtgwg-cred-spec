...
This specification is designed to work alongside the following companion specifications, both of which are themselves pre-publication:

- **[Trust Tasks](https://github.com/trustoverip/dtgwg-trust-tasks-tf/blob/main/SPEC.md)** — an editor's draft of the DTGWG Trust Tasks Task Force defining the framework for verifiable, transport-agnostic task exchanges between parties, including the document-identity (`id`) and `threadId` correlation mechanisms and the request/response and error-response document types that this specification's [Trust Task Context Binding](#trust-task-context-binding) relies on. Trust Tasks has not yet been published as a Working Group Approved Deliverable, but this specification's normative references to it are narrow and pinned to mechanisms already present in its current revision (see [References](#references)) — this specification does not depend on any future or unpublished feature of that framework. The two specifications are expected to continue co-evolving while both remain Working Drafts; implementers should track both repositories for changes affecting the binding.

- **DTG Core Trust Task Protocols** *(planned)* — may define DTG-specific Trust Task specifications (to offer, issue, request, present, and revoke DTG Core Credentials) satisfying the [Qualifying Trust Task Specifications](#qualifying-trust-task-specifications) profile this specification requires of any ceremony backing a `taskContext` value. Not yet published, and not a normative dependency of this specification — a governance framework MAY satisfy that profile with any qualifying Trust Task specification in the interim.
