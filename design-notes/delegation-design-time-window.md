...
> **Status (WD02).** This note is kept as the rationale record for the VDC as merged in [dtgwg-cred-spec#19](https://github.com/trustoverip/dtgwg-cred-spec/pull/19); the WG discussion it opened is [dtgwg-cred-tf#40](https://github.com/trustoverip/dtgwg-cred-tf/discussions/40). It does not describe the credential the specification defines. What WD02 took from it: the acceptance is required and constitutive (E2); single-hop is the default and `maxDepth` above zero is the only authorisation to re-delegate (§2, example 2); `credentialStatus` is conditional, with short validity and re-issuance the stated default and the status-lookup correlation cost recorded (§5, E5); the acceptance references the grant by digest and carries no `scope` (§6, second defect); per-principal delegate identifiers (E4); and chain disclosure as a privacy consideration (§2). What WD02 did not take: the six-field core and opt-in offline profile as the normative shape — the #40 thread endorsed single-hop as a strong default rather than the only model — and salted or SAID-based binders, which are cross-cutting and tracked in [dtgwg-cred-spec#38](https://github.com/trustoverip/dtgwg-cred-spec/issues/38). Section 6 below therefore describes the draft as it stood in August 2026, not the merged text.
